 ...
 */
contract HackedAuthorizer is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function rely(address usr, address aim) public {
        wards[usr][aim] = 1;
        emit Rely(usr, aim);
    }

    function can(address, address) external pure returns (bool) {
        revert("test");
        return true;
    }
